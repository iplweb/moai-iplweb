...
class OAIDC(object):
    ...
    def __init__(self, prefix, config, db):
        ...
        self.ns = {"oai_dc": "http://www.openarchives.org/OAI/2.0/oai_dc/", "dc": "http://purl.org/dc/elements/1.1/"}
    ...
    def __call__(self, element, metadata):
        ...
        OAI_DC = ElementMaker(namespace=self.ns["oai_dc"], nsmap=self.ns)
        DC = ElementMaker(namespace=self.ns["dc"])

        oai_dc = OAI_DC.dc()
        oai_dc.attrib["{%s}schemaLocation" % XSI_NS] = "%s %s" % (self.ns["oai_dc"], self.schemas["oai_dc"])
...
class MODS(object):
    ...
    def __init__(self, prefix, config, db):
        ...
        self.schemas = {"mods": "http://www.loc.gov/standards/mods/v3/mods-3-2.xsd"}
    ...
    def __call__(self, element, metadata):
        ...
        for ctype in ["author", "editor", "advisor"]:
            ...
            for contributor in contributor_data:
                unique_id = data["record"]["id"] + "_" + contributor.get("id", contributor["name"][0])
                if unique_id[0].isdigit():
                    unique_id = "_" + unique_id
                unique_id = unique_id.replace(":", "")
                name = MODS.name(MODS.displayForm(contributor["name"][0]), type="personal", ID=unique_id)
        ...
        if dgg:
            mods.append(
                MODS.name(
                    MODS.namePart(dgg[0]),
                    MODS.role(MODS.roleTerm("dgg", authority="marcrelator", type="code")),
                    type="corporate",
                )
            )
        ...
        if data["metadata"].get("language"):
            mods.append(
                MODS.language(MODS.languageTerm(data["metadata"]["language"][0], type="code", authority="rfc3066"))
            )
        ...
        for host in ["journal", "series"]:
            ...
            if issn:
                relitem.append(MODS.identifier("urn:issn:%s" % issn[0], type="uri"))
        ...
        if data["metadata"].get("date"):
            origin.append(MODS.dateIssued(data["metadata"]["date"][0], encoding="iso8601"))
