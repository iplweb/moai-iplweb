...
class IReadOnlyDatabase(Interface):
    ...
    def oai_query(
        offset=0, batch_size=20, sets=[], not_sets=[], filter_sets=[], from_date=None, until_date=None, identifier=None
    ):
        """Used by queries from the OAI server. Format returned should be the
        following:
...
class IFeedConfig(Interface):
    ...
    admins = Attribute("List of email addresses that can be contacted, for questions about the feed")
    ...
    sets_disallowed = Attribute("Objects served must not have one of these sets")
    filter_sets = Attribute(
        "Objects served must have one of these sets, besides the conforming to the (dis-)allowed sets"
    )
