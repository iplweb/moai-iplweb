...
def check_type(object, expected_type, unicode_keys=False, unicode_values=False, recursive=False, prefix="", suffix=""):
    object_type = type(object)
    if not isinstance(object, expected_type):
        raise TypeError(
            (
                '%s expected "%s", got "%s" %s' % (prefix, expected_type.__name__, object.__class__.__name__, suffix)
            ).strip()
        )
    if unicode_keys and object_type is dict:
        check_type(object.keys(), list, unicode_values=True, prefix=prefix, suffix=suffix)
...
class ProgressBar(object):
    ...
    def tick(self, count, total):
        ...
        self.write(("%5s%%[%-" + str(awidth) + "s] %" + str(lstot) + "d") % (perc, arrow, count))
