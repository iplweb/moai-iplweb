...
class MODS(object):
    ...
    def __call__(self, element, metadata):
        ...
        if data["metadata"].get("identifier"):
            mods.append(MODS.identifier(data["metadata"]["identifier"][0], type="uri"))
        ...
        if data["metadata"].get("title"):
            titleInfo = MODS.titleInfo(MODS.title(data["metadata"]["title"][0]))
            titleInfo.attrib["{%s}lang" % self.ns["xml"]] = data["metadata"].get("language", ["en"])[0]
        ...
        if data["metadata"].get("url"):
            location_el = MODS.location(
                MODS.url(data["metadata"]["url"][0], usage="primary display", access="object in context")
            )
            for asset in data["metadata"].get("asset", []):
                if asset.get("access") == "open":
                    location_el.append(MODS.url(asset["absolute_uri"], access="raw object"))
            mods.append(location_el)

        public_assets = [a for a in data["metadata"].get("asset", []) if a.get("access") == "open"]
        if len(public_assets) == 1:
            ...
            if asset.get("bytes"):
                try:
                    kbytes = re.sub(r"(\d{3})(?=\d)", r"\1,", str(int(asset["bytes"]) / 1024)[::-1])[::-1]

                    phys_descr_el.append(MODS.extent("Filesize: %s KB" % kbytes))
                except (ValueError, TypeError, KeyError):
                    pass  # Skip filesize display if data format is unexpected
        ...
        for ctype in ["author", "editor", "advisor"]:
            ...
            if data["metadata"].get("%s_data" % ctype):
                contributor_data = [s for s in data["metadata"]["%s_data" % ctype]]

            if not contributor_data:
                contributor_data = [{"name": [a]} for a in data["metadata"].get(ctype, [])]
            ...
            for contributor in contributor_data:
                ...
                name = MODS.name(MODS.displayForm(contributor_name), type="personal", ID=unique_id)
                ...
                if role:
                    ...
                    role = roles[ctype]
                name.append(MODS.role(MODS.roleTerm(role, type="code", authority="marcrelator")))
                mods.append(name)
            ...
            if dai_list:
                daiList = DAI.daiList()
                daiList.attrib["{%s}schemaLocation" % XSI_NS] = "%s %s" % (self.ns["dai"], self.schemas["dai"])

                for id, dai in dai_list:
                    daiList.append(DAI.identifier(dai[0].split("/")[-1], IDref=id, authority="info:eu-repo/dai/nl"))
        ...
        for corp in data["metadata"].get("corporate_data", []):
            roles = MODS.role()
            if corp.get("role"):
                roles.append(MODS.roleTerm(corp["role"], authority="marcrelator", type="text"))
            if corp.get("role_code"):
                roles.append(MODS.roleTerm(corp["role_code"], authority="marcrelator", type="code"))
            mods.append(MODS.name(MODS.namePart(corp["name"]), roles, type="corporate"))

        if data["metadata"].get("language"):
            lang_el = MODS.language(
                MODS.languageTerm(data["metadata"]["language"][0], type="code", authority="rfc3066")
            )
        ...
        for host in ["journal", "series"]:
            ...
            if issn:
                relitem.append(MODS.identifier(issn[0], type="issn"))
            host_uri = data["metadata"].get("%s_uri" % host)
            if host_uri:
                relitem.append(MODS.identifier(host_uri[0], type="uri"))
            ...
            if volume or issue or end_page or start_page:
                part = MODS.part()
                if volume:
                    part.append(MODS.detail(MODS.number(volume[0]), type="volume"))
                if issue:
                    part.append(MODS.detail(MODS.number(issue[0]), type="issue"))
            ...
            if data["metadata"].get("%s_publisher" % host):
                relitem.append(MODS.originInfo(MODS.publisher(data["metadata"]["%s_publisher" % host][0])))
        ...
        if data["metadata"].get("date"):
            origin.append(MODS.dateIssued(data["metadata"]["date"][0], encoding="w3cdtf"))
        ...
        for funder in funders:
            ...
            mods.append(
                MODS.name(
                    MODS.namePart(funder),
                    MODS.role(MODS.roleTerm("fnd", authority="marcrelator", type="code")),
                    ID=unique_id,
                    type="corporate",
                )
            )
        ...
        if projects:
            ...
            galList.attrib["{%s}schemaLocation" % XSI_NS] = "%s %s" % (self.ns["gal"], self.schemas["gal"])
        ...
        if info:
            ...
            if info.get("created"):
                record_info_el.append(MODS.recordCreationDate(info["created"], encoding="w3cdtf"))
            if info.get("changed"):
                record_info_el.append(MODS.recordChangeDate(info["changed"], encoding="w3cdtf"))
            mods.append(record_info_el)

        mods.attrib["{%s}schemaLocation" % XSI_NS] = "%s %s" % (self.ns["mods"], self.schemas["mods"])
...
class NL_MODS(MODS):
    ...
    def __call__(self, element, metadata):
        ...
        for el in element.xpath('./mods:mods/mods:originInfo/mods:dateIssued[@encoding="w3cdtf"]', namespaces=self.ns):
