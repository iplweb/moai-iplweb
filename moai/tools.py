...
def update_moai():
    ...
    if not os.path.isfile(config_path):
        sys.stderr.write(
            "No config_path file found at %s,\nStart script from other directory or use --config option\n" % config_path
        )
        sys.exit(1)
    ...
    if options.from_date:
        if "T" in options.from_date:
            ...
            fmt = "%Y-%m-%d"
        from_date = datetime.datetime(*time.strptime(options.from_date, fmt)[:6])
    else:
        from_date = None
    ...
    if not options.quiet and not options.verbose:
        ...
        print("Content provider returned %s new/modified objects" % count, file=sys.stderr)
        print(file=sys.stderr)
    ...
    for content_id in provider.get_content_ids():
        ...
        try:
            ...
            if options.debug:
                raise
            log.error("Error retrieving data %s from provider: %s" % (content_id, str(err)))
            error_count += 1
        ...
        try:
            ...
            if options.debug:
                raise
            log.error("Error converting data %s to content: %s" % (content_id, str(err)))
            error_count += 1
            progress.tick(count, total)
            continue
        ...
        try:
            database.update_record(content.id, content.modified, content.deleted, content.sets, content.metadata)
        except Exception as err:
            if options.debug:
                raise
            log.error("Error inserting %s into database: %s" % (content.id, str(err)))
            error_count += 1
            progress.tick(count, total)
            continue
    ...
    if error_count:
        msg = "%s error%s occurred during updating" % (error_count, {1: ""}.get(error_count, "s"))
        log.warning(msg)
        if not options.verbose and not options.quiet:
            print(msg, file=sys.stderr)
