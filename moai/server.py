...
class Server(object):
    ...
    def download_asset(self, req, url, config):
        ...
        for asset in self._db.get_assets(id):
            if asset["filename"] == filename or asset["md5"] == filename:
                break
        else:
            return req.send_status("404 File not Found", 'The asset "%s" does not exist' % filename)

        if not os.path.isfile(asset["path"]):
            return req.send_status("404 File not Found", 'The asset file "%s" does not exist' % filename)

        return req.send_file(asset["path"], asset["mimetype"].encode("ascii"))
    ...
    def allow_download(self, url, config):
        ...
        try:
            header, metadata, description = oai_server.getRecord("oai_dc", config.oai_id_prefix + id)
    ...
    def handle_request(self, req):
        if not req.url().startswith(self.base_url):
            return req.send_status(
                ...
                'The url "%s" does not start with base url "%s".' % (req.url(), self.base_url),
            )
        ...
        if len(urlparts) == 0:
            return req.send_status(
                "500 Internal Server Error", "No server was selected, please append server name to url."
            )
        ...
        if self.is_asset_url(url, self._config):
            if self.allow_download(url, self._config):
                ...
                return req.send_status("403 Forbidden", "You are not allowed to download this asset")
...
class FeedConfig(object):
    ...
    def __init__(
        ...
        self.base_asset_path = extra_args.get("base_asset_path", tempfile.gettempdir())
