    ...
    def _createHeader(self, record):
        ...
        for setspec in record["sets"]:
            if setspec in self.config.sets_deleted:
                deleted = True
                break
        return oaipmh.common.Header(None, record["id"], record["modified"], record["sets"], deleted)
...
def OAIServerFactory(db, config):
    ...
    for prefix in config.metadata_prefixes:
        metadata_registry.registerWriter(prefix, get_writer(prefix, config, db))

    return oaipmh.server.BatchingServer(
        OAIServer(db, config), metadata_registry=metadata_registry, resumption_batch_size=config.batch_size
    )
