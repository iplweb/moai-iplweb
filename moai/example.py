...
class ExampleContent(object):
    ...
    def update(self, path):
        ...
        for num, el in enumerate(xpath("//x:author"), 1):
            ...
            name = "%s %s" % (first, sur)
            author_data.append({"name": [name], "surname": [sur], "firstname": [first], "role": ["aut"]})

        self.metadata = {
            "identifier": ["http://example.org/data/%s" % id],
        ...
        self.sets = {"example": {"name": "example", "description": "An Example Set"}}

        access = xpath.string("//x:access")
        if access == "public":
            self.sets["public"] = {"name": "public", "description": "Public access"}
            self.metadata["rights"] = ["open access"]
        elif access == "private":
            self.sets["private"] = {"name": "private", "description": "Private access"}
